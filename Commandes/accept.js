...
module.exports.run = async (bot, message, args) => {
 ...
 try {
  ...
  if(!suggestionmessage.embeds[0]){
      return message.channel.send(`${police} Suggestion invalide`)
  };
  ...
  if(suggestionmessage.embeds[0].title !== `Nouvelle Suggestion`) {
    ...
    .setDescription(`${description}\n\n__**Approuvé par:**__ ${message.author.tag}\n__**Raison:**__ ${reason}`)
    .setFooter(footer)
    .setColor("#00ff37")
    ...
    return message.channel.send(`Approuvé avec succès la suggestion dans ${channel}!\n\n${suggestionmessage.url}`)
    .then((s)=>{
